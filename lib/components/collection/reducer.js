...
var select = function select(state, action) {
  return _extends({}, state, {
    selected: !_lodash2.default.includes(state.selected, action.id) ? [].concat(_toConsumableArray(state.selected), [action.id]) : state.selected.filter(function (id) {
      return id !== action.id;
    })
  });
};
...
var selectAll = function selectAll(state, action) {
  ...
  return _extends({}, state, {
    selected: _lodash2.default.isEqual(all.sort(), state.selected.sort()) ? [] : all
  });
};
